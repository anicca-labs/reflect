  ...
  const translateAuthError = useCallback(
    (message: string): string => {
      ...
      if (lower.includes('email not confirmed'))
        return t`Please confirm your email before signing in`;
        ...
        if (!cancelled)
          setAuthError(
            err.code === statusCodes.PLAY_SERVICES_NOT_AVAILABLE
              ? t`Google Play Services not available`
        ...
        <YStack gap="$0">
          ...
          <FormField error={passwordError}>
            <PasswordInput
              ...
              onSubmitEditing={() =>
                isReady ? handleSubmit() : confirmPasswordRef.current?.focus()
              }
          ...
          {/* Shown in BOTH modes. These used to be sign-in only, so a guest who hit
              the entry limit and tapped "Sign up" lost the one-tap options and was
              forced through email + password + confirm + an email round-trip that
              takes them out of the app mid-thought. signInWithIdToken creates the
              account for a new user just the same — hiding them was purely a
              rendering condition, and it sat on the app's main conversion path. */}
          <YStack gap="$3" mt="$6" opacity={socialLoading ? 0.6 : 1}>
            ...
            !!process.env.EXPO_PUBLIC_ANDROID_APPLE_SIGN_IN_CLIENT_ID ? (
              ...
              >
                ...
                />
                <LabelLg color={isDark ? '$black' : '$white'}>
