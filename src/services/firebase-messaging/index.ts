...
const scheduleMemoryNotifications = async (
  ...
  if (existingJson) {
    ...
    await Promise.all(ids.map((id) => ExpoNotifications.cancelScheduledNotificationAsync(id).catch(() => {})))
  }
  ...
  const oldEntries = entries.filter((e) => now.getTime() - new Date(e.created_at).getTime() >= minAgeMs)
